...
namespace BooTao2.Content.Projectiles.Beehunter
{
	...
	public class BeehunterProj : ModProjectile
	{
		...
		public override void AI() {
			...
			//Dust.NewDust(Projectile.position, 0, 0, 140, 0, 0, 150, default, 1f);
		...
		public override void ModifyHitNPC (NPC target, ref NPC.HitModifiers modifiers) {
			...
			if (player.GetModPlayer<BooTaoPlayer>().JackieHolding > 0 && player.GetModPlayer<BooTaoPlayer>().JackieSP >= 6) {
				modifiers.ModifyHitInfo += (ref NPC.HitInfo hitInfo) => {
					hitInfo.Damage = (int)(hitInfo.Damage * 2.5);
				};
			}
			if (player.GetModPlayer<BooTaoPlayer>().JackieSP >= 7) {
				player.GetModPlayer<BooTaoPlayer>().JackieSP = -1;
			}
			//
			if (player.GetModPlayer<BooTaoPlayer>().BeehunterHolding > 0) {
				modifiers.ModifyHitInfo += (ref NPC.HitInfo hitInfo) => {
					hitInfo.Damage += player.GetModPlayer<BooTaoPlayer>().BeehunterStacks;
				};
			}
		}
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			...
			if (player.GetModPlayer<BooTaoPlayer>().JackieHolding > 0) {
				player.GetModPlayer<BooTaoPlayer>().JackieSP++;
			}
			if (player.GetModPlayer<BooTaoPlayer>().BeehunterHolding > 0) {
				if (player.GetModPlayer<BooTaoPlayer>().BeehunterStacks < 6) {
					player.GetModPlayer<BooTaoPlayer>().BeehunterStacks++;
				}
			}
			else {
