...
{
	public class SilverWolfMaterial : ModItem
	{
		...
		public override void SetDefaults() {
			...
			Item.value = Item.buyPrice(platinum: 1); // Item.buyPrice & Item.sellPrice are helper methods that returns costs in copper coins based on platinum/gold/silver/copper arguments provided to it.
		...
		public override void AddRecipes() {
			...
			if (ModLoader.TryGetMod("CalamityMod", out Mod calamityMod) && calamityMod.TryFind("ShadowspecBar", out ModItem ShadowspecBar) ) {
				recipe.AddIngredient(ShadowspecBar.Type, 99);
			}
			else {
				recipe.AddIngredient(ItemID.LunarBar, 499);
			}
